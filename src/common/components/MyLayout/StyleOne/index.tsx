...
export default ({
  ...
  const [defaultSelectedKeys] = useState<string[]>(() =>
    getDefaultSelectedKeysFromUrl(initPathname, auth.permissions?.children),
  );

  // 初始化主菜单数据
  const [mainMenus] = useState<MenuItemType[]>(() =>
    getMainMenus(auth.permissions?.children),
  );

  // 初始化子菜单数据
  const [subMenus, setSubMenus] = useState<MenuItemType[]>(() =>
    getSubMenusFromSelectedMainMenu(
      auth.permissions?.children,
